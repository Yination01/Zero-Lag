...
| Thing | Value |
|---|---|
| Tip of this branch | Expo React Native Android app with local readiness history, native HUD controls, truth-first permission flows, selected text-free neon signal-pulse branding, and API 36 Play candidate source. Build 12 is in progress through the GitHub-local production AAB workflow from `df205cf`. It has no AAB or device result yet. Build 11 remains the latest finished preview APK and predates the current API 36 source. |
...
| Current Play candidate workflow | [Build 12 workflow run](https://github.com/Yination01/Zero-Lag/actions/runs/33731736686), workflow `349139383`, was accepted at `2026-09-03T08:08:15Z` from API 36 commit `df205cf` and is in progress. It will make a signed `zero-lag.aab` plus `play-candidate-12` only if every source and Android gate succeeds. It never submits to Google Play. No AAB, release asset, archive evidence, Console upload, installation, or device test exists yet. |
...
| `.build-state.json` | The most recently dispatched Android artifact record. Currently in-progress Build 12 production AAB candidate from API 36 commit `df205cf`, with no AAB or device evidence yet. Build 11 remains the prior finished preview APK. |
...
| `play/` | Closed-testing readiness record, truthful store listing copy, screenshot capture guide, staged privacy-policy template, and Console handoff. Public policy deployment is blocked pending a real contact. |
