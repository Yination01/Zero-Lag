...
| Thing | Value |
|---|---|
| Tip of this branch | Expo React Native Android app with local readiness history, native HUD controls, truth-first permission flows, selected text-free neon signal-pulse branding, and API 36 Play candidate source ready for the named Build 12. Build 11 completed from the prior Expo 51 source and published an APK, but it has not yet been installed or device-tested. Build 12 cannot dispatch until this source is pushed. |
...
| `.build-state.json` | The most recently dispatched APK record. Currently finished Build 11 from branding commit `e77c13e`, with a verified prerelease APK but no installation or device evidence yet. Build 12 has been named but is not dispatched until the current API 36 source reaches `main`. |
