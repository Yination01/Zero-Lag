...
| Latest GitHub APK workflow | [Build 11 workflow run](https://github.com/Yination01/Zero-Lag/actions/runs/33694402846) was accepted at `2026-09-02T23:17:28Z` from branding commit `e77c13e` and is in progress. It will use GitHub-local EAS to create `zero-lag.apk` and the `preview-11` prerelease only if every source and Android build step succeeds. There is no Build 11 APK, release asset, archive evidence, installation, or device test yet. |
...
| Last finished APK | [Build 10 `zero-lag.apk`](https://github.com/Yination01/Zero-Lag/releases/download/preview-10/zero-lag.apk) finished successfully from `1fd1b72` through GitHub-local EAS. It predates the launcher, adaptive-icon, and splash branding source, and it is not installed or device-tested. Build 8 remains installed, but its real-device Settings, permission, HUD, notification, and network checks have not run. |
...
| `.build-state.json` | The most recently dispatched APK record. Currently queued Build 11 from branding commit `e77c13e`, with no APK or device evidence yet. Build 10 contains the prior finished prerelease asset but not the new branding. Build 9 queue state and Build 8 evidence remain in decisions and sitemap. |
