...
| Thing | Value |
|---|---|
| Tip of this branch | Expo React Native Android app with local readiness history, native HUD controls, truth-first permission flows, selected text-free neon signal-pulse branding, API 36 Play candidate source, and terminal Build 12 evidence. GitHub-local Build 12 succeeded from `df205cf`, publishing a technically verified AAB candidate. It has not been uploaded to Play or device-tested. Build 11 remains the latest finished preview APK and predates the current API 36 source. |
...
| Latest Play candidate workflow | [Build 12 workflow run](https://github.com/Yination01/Zero-Lag/actions/runs/33731736686), workflow `349139383`, completed successfully at `2026-09-03T08:16:47Z` from API 36 commit `df205cf`. It passed the source, local EAS AAB, ZIP, JAR signature, artifact, and prerelease steps without submitting to Google Play. The published [`zero-lag.aab`](https://github.com/Yination01/Zero-Lag/releases/download/play-candidate-12/zero-lag.aab) is 39,137,186 bytes, SHA-256 `dee12bc6a4adcfd6bc172a93d0afae724e80ce045fbd26dfc8df72896350448e`, with built target SDK 36. [Build 12 evidence](docs/play/BUILD_12_EVIDENCE.md) documents independent checks. No Console upload, installation, or device test exists. |
| Latest finished GitHub APK workflow | [Build 11 workflow run](https://github.com/Yination01/Zero-Lag/actions/runs/33694402846) completed successfully at `2026-09-02T23:22:26Z` from branding commit `e77c13e`. Its GitHub-local EAS job passed every required step and published [`zero-lag.apk`](https://github.com/Yination01/Zero-Lag/releases/download/preview-11/zero-lag.apk) in the [`preview-11` prerelease](https://github.com/Yination01/Zero-Lag/releases/tag/preview-11). The 62,737,907-byte APK passed archive integrity checks but is not installed or device-tested. It predates the current API 36 preparation source. |
...
| Suite | `npm test` runs agent-docs, agent-rules, legal, dependency-lock, the GitHub APK and Play candidate AAB workflow gates, TypeScript, and TS app tests. |
...
| `.build-state.json` | The most recent Android artifact record. Build 12 production AAB candidate succeeded from API 36 commit `df205cf`, with GitHub release, checksum, ZIP, bundle, signature, and manifest evidence recorded. It has no Console upload or device evidence. Build 11 remains the prior finished preview APK. |
| `package.json` | Expo 54, React Native 0.81, reproducible dependencies, and `npm test`, the local gate. |
| `app.json` / `eas.json` | Expo config and EAS Android build profiles. `app.json` binds the 1024 px launcher icon, dark contain-mode splash image, Android adaptive foreground and background, and explicit Android 36 build properties. `eas.json` retains separate preview APK and production AAB profiles. |
| `assets/` | Editable SVG masters and 1024 px PNG delivery assets for the text-free neon signal-pulse launcher icon, transparent adaptive-icon foreground, and transparent splash mark. `assets/play-store/` holds the opaque 512 px listing icon and 1024 by 500 feature graphic. |
...
| `.github/workflows/play-aab.yml` | Manual Play candidate workflow. It requires a numeric unused build number of 12 or higher and `EXPO_TOKEN`, runs the production EAS profile locally, verifies the signed `zero-lag.aab`, then retains it as an artifact and `play-candidate-N` prerelease asset. It never submits to Google Play. |
...
| `play-aab-workflow.cjs` | Static gate for the numbered production-AAB candidate route: manual and tag guards, Expo secret, locked source gate, API-preserving metadata, local EAS bundle, archive and signature checks, artifact, prerelease candidate, and no Play submission. |
...
| `DEVICE_TEST_PLAN.md` | Real Android device validation plan for preview APKs and Play-delivered candidate builds, including launcher, adaptive icon, and splash checks. |
| `play/` | Closed-testing readiness record, Build 12 technical evidence, truthful store listing copy, screenshot capture guide, staged privacy-policy template, and Console handoff. Public policy deployment is blocked pending a real contact. |
